...
import {
  applySapDistributorMasterRow,
  normalizeSapDistributorMasterRow,
} from "@/lib/integrations/sap-distributor-master";
import {
...
type DistributorMasterImportPayload = {
  rows?: unknown;
};
...
export async function POST(request: Request) {
  try {
    ...
    const body = parseJsonBody<DistributorMasterImportPayload>(
      await request.json(),
    );
    ...
    if (rows.length === 0) {
      throw new ApiError("At least one distributor master row is required.", 400);
    }
    ...
    const feed = scaffold.feeds.find(
      (entry) => entry.feedType === "distributor_master",
    );
    if (!feed) {
      throw new ApiError("SAP distributor master feed is not configured.", 500);
    }

    if (!feed.enabled) {
      throw new ApiError("SAP distributor master feed is disabled.", 409);
    }
    ...
    for (const [index, row] of rows.entries()) {
      ...
      const { normalized, errors } = normalizeSapDistributorMasterRow(row);
      const externalRecordKey = normalized?.externalDistributorCode ?? null;
      ...
      try {
        const result = await db.$transaction(async (tx) => {
          const applied = await applySapDistributorMasterRow(tx, {
            organizationId,
            row: normalized,
            rawPayload: row as Prisma.InputJsonValue,
          ...
          await createStagingRecord(tx, {
            ...
            organizationId,
            feedType: "distributor_master",
            externalRecordKey: normalized.externalDistributorCode,
            rowNumber,
            rawPayload: row as Prisma.InputJsonValue,
            normalizedPayload:
              normalized as unknown as Prisma.InputJsonValue,
            status: "applied",
            domainTargetType: "organization",
            domainTargetId: applied.organizationId,
          });
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Failed to apply distributor master row.";
