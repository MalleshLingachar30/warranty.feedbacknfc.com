...
import {
  applySapSerializedDispatchRow,
  normalizeSapSerializedDispatchRow,
} from "@/lib/integrations/sap-serialized-dispatch";
import {
...
type SerializedDispatchImportPayload = {
  rows?: unknown;
};
...
export async function POST(request: Request) {
  try {
    ...
    const body = parseJsonBody<SerializedDispatchImportPayload>(
      await request.json(),
    );
    ...
    if (rows.length === 0) {
      throw new ApiError(
        "At least one serialized dispatch row is required.",
        400,
      );
    }
    ...
    const feed = scaffold.feeds.find(
      (entry) => entry.feedType === "serialized_dispatch",
    );
    if (!feed) {
      throw new ApiError("SAP serialized dispatch feed is not configured.", 500);
    }

    if (!feed.enabled) {
      throw new ApiError("SAP serialized dispatch feed is disabled.", 409);
    }
    ...
    for (const [index, row] of rows.entries()) {
      ...
      const { normalized, errors } = normalizeSapSerializedDispatchRow(row);
      const externalRecordKey =
        normalized?.serialNumber ?? `row-${rowNumber.toString()}`;
      ...
      try {
        const result = await db.$transaction(async (tx) => {
          const applied = await applySapSerializedDispatchRow(tx, {
            organizationId,
            row: normalized,
            rawPayload: row as Prisma.InputJsonValue,
          ...
          await createStagingRecord(tx, {
            ...
            normalizedPayload:
              normalized as unknown as Prisma.InputJsonValue,
            status:
              applied.status === "applied" ? "applied" : "validated",
            domainTargetType: "erp_serialized_dispatch_record",
            domainTargetId: applied.erpSerializedDispatchRecordId,
          });
        ...
        if (result.status === "applied") {
          ...
          pendingMatchCount += 1;
          errorSummary.pending_match = (errorSummary.pending_match ?? 0) + 1;
        }
      } catch (error) {
        failedRowCount += 1;
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Failed to apply serialized dispatch row.";
    ...
    const status =
      failedRowCount > 0
        ? appliedRowCount > 0 || pendingMatchCount > 0
